...
set "PACKAGE_LIST=%SCRIPT_DIR%\package_list.txt"
set "CONNECTION_FILE=%SCRIPT_DIR%\tnsnames.txt"
set "OUTPUT_DIR=%SCRIPT_DIR%"

if not "%~1"=="" set "PACKAGE_LIST=%~1"
if not "%~2"=="" set "CONNECTION_FILE=%~2"
if not "%~3"=="" set "OUTPUT_DIR=%~3"

java -cp "%CP%" ExportOraclePackageDdlsFromList "%PACKAGE_LIST%" "%CONNECTION_FILE%" "%OUTPUT_DIR%"
